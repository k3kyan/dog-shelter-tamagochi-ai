...
# sentence-transformers runs locally here (pipeline runs on my machine, not Lambda)
# so torch is fine. only Lambda needs to avoid torch.
...
# -> chunk + generate context with Groq
...
def chunk(articles: list[str]) -> list[str]:
    ...
    for article in articles:
        ...
        for chunk_text in chunks: # add the chunks to all_chunks list, still keeping the other metadata for better context
            # append chunks for embedding
            all_chunks.append({
                'chunk': chunk_text, #for embedding + stored as metadata in ChromaDB for debugging retrieval quality
            ...
            chunk_no = chunk_no + 1
            print(f"Processed chunk: {chunk_text[:50]}")
...
# generate context sentence for a chunk using Groq
...
# (old) 4. Embed chunks and store in ChromaDB
# THE CORE OF A RAG PIPELINE!!!!!!
def embed_in_pinecone(contextualized_chunks):
    ...
    # Set up Pinecone connection (i set up the db on pinecone website already)
    pc = Pinecone(api_key=os.getenv('PINECONE_API_KEY'))
    index_name = os.getenv('PINECONE_INDEX', 'dog-shelter-tamagochi-ai')
    index = pc.Index(index_name)

    stats = index.describe_index_stats()

    # if Pinecone already has data, skip re-embedding to save api calls and time
    if stats.total_vector_count > 0:
        print(f"Pinecone already has {stats.total_vector_count} vectors. Skipping embedding.")
        print(f"Delete all vectors in the '{index_name}' index to force re-embed.")
        return

    # --------------------- actual processing ----------------------
    # make a list of the sentences to embed (context + chunk)
    # NOTE: unlike embed.py, this wont be in the metadata bc we only need the documents for EMBEDDING the vectors. just having these as separate metadata instead of combined this time.
    documents = []
    ...
    # this for loop absorbed the id and metadata for loops from the embed.py file
    BATCH_SIZE = 50
    for i in range(0, len(documents), BATCH_SIZE):
        # sections in batches
        batch_docs = documents[i:i + BATCH_SIZE]
        batch_chunks = contextualized_chunks[i:i + BATCH_SIZE]

        # generate embeddings (!!!!!) (converts text -> vectors (embeddings) yay!!) 
        embeddings = model.encode(batch_docs).tolist()

        vectors = []
        # Note: change from embed.py: 
        for j in range(len(batch_docs)):
            vectors.append({
                'id': f"chunk_{i + j}", # generate unique id's for each chunk (bc pinecone requires an ID for each document)
                'values': embeddings[j], #the embedding vectors! yay!!
                'metadata': { # metadata for each chunk
                    'chunk': batch_chunks[j]['chunk'],
                    'context': batch_chunks[j]['context'],
                    'source_url': batch_chunks[j]['source_url'],
                }
            })

        # store in Pinecone (aka add everything to the chromadb vector database) (allows semantic search!! yayay)
        index.upsert(vectors=vectors)

        # print progress in terminal as program runs
        print(f"Upserted batch {i // BATCH_SIZE + 1} / {(len(documents) - 1) // BATCH_SIZE + 1}")  

    print(f"\nPinecone complete: {len(documents)} vectors upserted to '{index_name}'")
...
if __name__ == '__main__':
    ...
    embed_in_pinecone(contextualized_chunks)
    # embed_in_chromadb(contextualized_chunks) #LOCAL